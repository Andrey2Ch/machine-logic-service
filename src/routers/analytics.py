...
async def get_lot_analytics(lot_id: int, db: Session = Depends(get_db_session)):
    ...
    try:
        ...
        # 🔧 ИСПРАВЛЕНО 2026-01-18: "Принято" = recounted_quantity (пересчитано кладовщиком)
        # Считаем ТОЛЬКО оригинальные батчи (parent_batch_id IS NULL)
        # После ОТК: родитель (archived) содержит recounted_quantity от склада,
        # дочерние батчи (good/defect) наследуют warehouse_received_at но recounted=0
        # Поэтому считаем только оригиналы, чтобы избежать двойного подсчёта
        warehouse_batches = [
